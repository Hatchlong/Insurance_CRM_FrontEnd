...
export class MakeListComponent {
  ...
  async getAllmakeDetails(filter: any, page: any, itemsPerPage: any) {
    try {
      const result: any = await this.makeSer.getAllmakeDetailsPageFilter(filter, page, itemsPerPage)
      if (result.status === '1') {
  ...
  handleFilter(event: any) {
    ...
    this.makeDetails = this.allmakeDetails.filter((obj: any) =>
      ((obj.makeId.toUpperCase()).includes(filterValue) || (obj.description.toUpperCase()).includes(filterValue)))
  }
  filterData() {
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          data.disable = true
          const result: any = await this.makeSer.updatemakeDetail(data);
          if (result.status === '1') {
